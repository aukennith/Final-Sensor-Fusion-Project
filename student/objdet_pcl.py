...
def show_pcl(pcl): 
    ...
    vis_lpc.create_window (window_name='Open3D', width=1280, height=720, left=50, top=50, visible=True)
...
def show_range_image(frame, lidar_name):
    ...
    img_range_intensity = ri_intensity.astype(np.uint8)
    
    # step 6 : stack the range and intensity image vertically using np.vstack and convert the result to an unsigned 8-bit integer
    img_range_intensity = np.vstack((img_range, img_range_intensity))
...
def bev_from_pcl(lidar_pcl, configs):
    ...
    lidar_pcl_cpy =  np.copy(lidar_pcl) #lidar_pcl_copy()
